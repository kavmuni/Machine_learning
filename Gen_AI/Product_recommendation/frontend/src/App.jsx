...
export default function App() {
  ...
  const handleRecommend = async () => {
    ...
    try {
      const r = await axios.post(`${API}/recommend`, { product: selectedProduct, method, top_n: 10 })
  ...
  const handleCompare = async () => {
    ...
    try {
      const r = await axios.post(`${API}/compare`, { product: selectedProduct, top_n: 8 })
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-5">
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-xl shadow-sm overflow-hidden">
          <div className="bg-[#F9FAFB] border-b border-[#E5E7EB] px-5 py-3">
            ...
            <p className="text-xs text-[#8D99A5]">Select a product and choose an embedding method</p>
          </div>
          <div className="p-5 grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div>
              <MovieSearch movies={products} selected={selectedProduct} onSelect={setSelectedProduct} />
            </div>
          ...
          <div className="px-5 py-4 border-t border-[#E5E7EB] bg-[#F9FAFB] flex flex-col sm:flex-row gap-3">
            ...
            >
              {loading ? (
                <span className="pulse-glow">Finding similar products…</span>
              ) : (
            ...
            <button
              ...
              disabled={!selectedProduct || comparing}
            ...
            >
              {comparing ? (
                <span className="pulse-glow">Running all 6 algorithms…</span>
              ) : (
          ...
          <div className="bg-white border border-[#E5E7EB] rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="p-5">
              {tab === 'recommend' && recommendations && (
                <MovieGrid recommendations={recommendations} method={method} queryMovie={selectedProduct} />
              )}

              {tab === 'compare' && comparisonData && (
                <div className="space-y-6">
                  <ComparisonChart comparisonData={comparisonData} queryMovie={selectedProduct} />

                  {/* Per-method mini grids */}
                  {Object.entries(comparisonData).map(([m, recs]) => {
                    const color = {
                      bow:'#DC2626', tfidf:'#D97706', word2vec:'#059669', glove:'#4F46E5', fasttext:'#DB2777', bge_m3:'#7C3AED'
                    }[m]
                    const label = {
                      bow:'Bag of Words', tfidf:'TF-IDF', word2vec:'Word2Vec (Google News)',
                      glove:'GloVe', fasttext:'FastText (Wiki News)', bge_m3:'BGE-M3 (BAAI)',
                    }[m]
                    return (
                      <div key={m}>
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="p-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-3">
            {[
              ...
              { name: 'BGE-M3',       step: 'BAAI multi-lingual 1024d vectors → cross-lingual semantic sim', color: '#7C3AED', bg: '#F3E8FF' },
            ].map((m) => (
