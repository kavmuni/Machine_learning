...
parquet_path = os.path.join(os.path.dirname(__file__), "data", "imdb_top_1000.parquet")
...
print(f"   Sample tokens from first product: {corpus[0][:10]}")
...
print("\n[1/2] Training Word2Vec on product corpus...")
...
try:
    sample_vec = w2v_model.wv["Bluetooth"]  # vector for a specific product name
    print(f"   'Bluetooth' vector shape: {sample_vec.shape} (first 5 dims: {sample_vec[:5].round(3)})")
    similar = w2v_model.wv.most_similar("Speaker", topn=5)
    print(f"   Words most similar to 'Speaker': {[w for w, _ in similar]}")
...
print("\n[2/2] Training FastText on product corpus...")
...
try:
    print(f"   'headset' vector shape: {ft_model.wv['headset'].shape}")
...
try:
    print(f"   'watch' most similar: {wv.most_similar('watch', topn=3)}")
except KeyError:
    print("   ('watch' not in vocabulary)")
